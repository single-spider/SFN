...
from ..training.train_sfms import SFMSActorCritic, _require_torch, make_sfms_state
...
def _obs_to_state(obs: dict, vsn: VirtualSensorNetwork, mask_source: str, device: str):
    ...
    with torch.no_grad():
        ...
        return make_sfms_state(out)


def load_sfms_policy(
...
def evaluate_sfms(
    ...
    """Evaluate a deterministic actor-mean SFMS policy."""
    torch, _ = _require_torch()
    ...
    try:
        ...
        for spec in episode_schedule(env.shapes, total_episodes, seed):
            ...
            while not (terminated or truncated):
                ...
                with torch.no_grad():
                    state = _obs_to_state(obs, vsn, mask_source, str(dev))
            ...
            records.append(
                {
                    ...
                    **initial,
                    ...
                    "termination_reason": info.get(
                        "termination_reason",
                        "success" if info.get("success") else "truncated",
                    ),
                    "insertion_depth_mm": info.get("insertion_depth_mm"),
                    "collision_failure": info.get("collision_failure"),
                    "residual_xy_error_mm": info.get("residual_xy_error_mm"),
                    "residual_yaw_error_deg": info.get("residual_yaw_error_deg"),
